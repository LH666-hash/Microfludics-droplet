...
def mean_absolute_percentage_error2(y_obs, y_pred): #for when the MAPE doesnt need reshaping
    ...
    return  np.mean(np.abs((y_obs - y_pred) / y_obs)) * 100
model = Sequential()
model.add(Dense(units = 512, activation = 'relu', input_dim = 16 , name='scratch_dense_1', kernel_regularizer=regularizers.l2(0.001)))
...
dataset = pd.read_csv("D:\double modle\double_droplet.csv",encoding="gbk")
X = dataset.iloc[:,0:16]
Y = dataset.iloc[:,-1]
...
results = pd.DataFrame(results,
                               columns=['train_error', 'train_mse', 'test_error',
                                         'test_mse'])
results.to_excel("D:\double modle\double_droplet_design.xlsx")
Y_test=list(Y_test)
...
test_results.to_excel("D:\double modle\double_droplet_design -.xlsx")
model.save('D:\double modle6.h5')