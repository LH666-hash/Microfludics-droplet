...
# 定义输入层
input_layer = Input(shape=(14,), name='input')
...
x = Dense(units=128, activation='relu', name='shared_dense_4', kernel_regularizer=regularizers.l2(0.001))(x)

# 定义两个输出层
output1 = Dense(units=1, name='output1')(x)
output2 = Dense(units=1, name='output2')(x)
output3 = Dense(units=1, name='output3')(x)
# 定义模型
...
model.compile(optimizer=adam,
              ...
              metrics={'output1': ['mean_squared_error', rmse, r_square],
                       ...
                       'output3': ['mean_squared_error', rmse, r_square]})

# 读取数据
dataset = pd.read_csv("D:\py\double droplet_data.csv", encoding="gbk")
X = dataset.iloc[:, 0:14]
Y1 = dataset.iloc[:, -1]  # 假设第一个输出是最后一列
Y2 = dataset.iloc[:, -2]  # 假设第二个输出是倒数第二列
Y3 = dataset.iloc[:, -3]
# 划分训练集和测试集
...
# 训练模型
result = model.fit(X_train, {'output1': Y1_train, 'output2': Y2_train, 'output3': Y3_train},
                   validation_data=(X_test, {'output1': Y1_test, 'output2': Y2_test,'output3': Y3_test}),
                   batch_size=32, epochs=3000)

# 预测
y_pred_train1, y_pred_train2,y_pred_train3 = model.predict(X_train)
y_pred1, y_pred2,y_pred3 = model.predict(X_test)
